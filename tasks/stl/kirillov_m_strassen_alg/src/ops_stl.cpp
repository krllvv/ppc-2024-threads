...
std::vector<double> kirillov_stl::strassen(const std::vector<double>& A, const std::vector<double>& B, int n) {
  if ((n == 0) || ((n & (n - 1)) != 0)) {
  ...
  threads.emplace_back([&]() { p1 = strassen(add(A11, A22), add(B11, B22), half); });
  threads.emplace_back([&]() { p2 = strassen(add(A21, A22), B11, half); });
  threads.emplace_back([&]() { p3 = strassen(A11, sub(B12, B22), half); });
  threads.emplace_back([&]() { p4 = strassen(A22, sub(B21, B11), half); });
  threads.emplace_back([&]() { p5 = strassen(add(A11, A12), B22, half); });
  threads.emplace_back([&]() { p6 = strassen(sub(A21, A11), add(B11, B12), half); });
  threads.emplace_back([&]() { p7 = strassen(sub(A12, A22), add(B21, B22), half); });
...
std::vector<double> kirillov_stl::joinMatrices(const std::vector<double>& A11, const std::vector<double>& A12,
...
void kirillov_stl::splitMatrix(const std::vector<double>& A, std::vector<double>& A11, std::vector<double>& A12,
...
std::vector<double> kirillov_stl::add(const std::vector<double>& A, const std::vector<double>& B) {
  int n = A.size();
...
std::vector<double> kirillov_stl::sub(const std::vector<double>& A, const std::vector<double>& B) {
  int n = A.size();
  std::vector<double> C(n);
  for (int i = 0; i < n; i++) {
    C[i] = A[i] - B[i];
...
std::vector<double> kirillov_stl::mul(const std::vector<double>& A, const std::vector<double>& B, int n) {
  if (n == 0) {
...
std::vector<double> kirillov_stl::generateRandomMatrix(int n) {
...
bool StrassenMatrixMultParallelSTL::pre_processing() {
  internal_order_test();
...
bool StrassenMatrixMultParallelSTL::validation() {
  internal_order_test();
  return taskData->inputs_count[0] == taskData->inputs_count[1] &&
         taskData->inputs_count[0] == taskData->outputs_count[0];
}

bool StrassenMatrixMultParallelSTL::run() {
  internal_order_test();
  C = strassen(A, B, n);
  return true;
}

bool StrassenMatrixMultParallelSTL::post_processing() {
  internal_order_test();
